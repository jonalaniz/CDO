...
final class SAService: CRUDService {
    ...
    func fetchAll() async throws -> [Model] {
        let urlWithEndpoint = URL(string: baseURLString + Endpoint.serviceAuthorizations.path)!
        return try await apiManager.fetchResource(urlWithEndpoint)
    }

    func fetch(id: Int) async throws -> Model {
        let urlWithEndpoint = URL(string: baseURLString + Endpoint.serviceAuthorization(id).path)!
        return try await apiManager.fetchResource(urlWithEndpoint)
    }
